...
final class ContentTypeListener
{
    ...
    private function generateMigration(\Ibexa\Contracts\Core\Repository\Values\ContentType\ContentType|\Ibexa\Contracts\Core\Repository\Values\ContentType\ContentTypeDraft $contentType, string $mode): void
    {
        ...
        try {
            ...
            $this->logger->info('Migration generate process finished (' . $mode . ')', ['name' => $name, 'code' => $code, 'output' => $this->migrationRunner->getOutput(), 'error' => $this->migrationRunner->getErrorOutput()]);
            if ($code == 0) {
                if ($this->mode === 'ibexa') {
                ...
                $this->logger->error('Migration generation failed', ['code' => $code, 'error' => $this->migrationRunner->getErrorOutput()]);
            }
        } catch (\Throwable $e) {
