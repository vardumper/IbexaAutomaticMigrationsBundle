...
final class UserListener implements EventSubscriberInterface
{
    ...
    private function generateMigration(\Ibexa\Contracts\Core\Repository\Values\User\User $user, string $mode): void
    {
        ...
        try {
            ...
            $this->logger->info('User migration generate process finished (' . $mode . ')', ['name' => $name, 'code' => $code, 'output' => $this->migrationRunner->getOutput(), 'error' => $this->migrationRunner->getErrorOutput()]);
            if ($code == 0) {
                if ($this->mode === 'ibexa') {
                ...
                $this->logger->error('User migration generation failed', ['code' => $code, 'error' => $this->migrationRunner->getErrorOutput()]);
            }
        } catch (\Throwable $e) {
